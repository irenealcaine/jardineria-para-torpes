          ...
          PlantsData.map((data, id) => {
            return (
              ...
              <div key={id} className="border border-green-400 shadow-xl rounded-lg md:w-80 w-full bg-white">
                ...
                <div className={` text-center py-1 border-2 rounded-lg  m-0.5 mt-4  ${data.category === 'Flor' ? 'border-rose-600 bg-rose-100' : data.category === 'Decorativa' ? 'border-fuchsia-600 bg-fuchsia-100' : data.category === 'Culinaria' ? 'border-lime-600 bg-lime-100' : 'border-orange-600 bg-orange-100'}`}>{data.category}</div>
              </div>
            )
          })
        }
